...
基于 **28 题固定测试集**（按硬度 5 / 颜色 5 / 酸度 5 / 采收期 4 / 糖度 4 / 通用方法 5 分组），LLM-as-Judge 自动评分（DeepSeek-chat 担任 judge，给"机制准确性"维度评分；其余维度规则化打分），对比四种配置：

| 配置 | 基因召回(4) | 机制准确(3) | 引用质量(2) | 证据分级(1) | **总分(10)** |
|------|:----------:|:----------:|:----------:|:----------:|:-----------:|
| A0 No-RAG（纯 LLM） | 2.32 | 2.18 | 0 | 0 | **4.50** |
| A1 Papers-only | 3.18 | 1.57 | 2.00 | 0.93 | **7.68** |
| A2 Genes-only | **3.54** | **2.18** | 2.00 | **0.93** | **8.64** |
| **A3 Hybrid（完整系统）** | **3.54** | 2.11 | 2.00 | 0.86 | **8.50** |

A3 完整系统分性状得分（由高到低）：颜色 9.20、硬度 9.00、酸度 8.60、采收期 8.25、通用 8.00、糖度 7.75。

> 完整系统总分 8.50/10，相比未做 prompt + 检索优化的初版（7.07）提升约 20%。  
> A2 (Genes-only) 总分略高于 A3 (Hybrid) 是因为部分论文检索结果在 rerank 后排到了基因证据之前，稀释了"机制准确性"。但 A3 在性状覆盖广度与引用多样性上仍优于 A2。

完整评测协议、测试集、原始结果与 LLM judge 评分见 [`workspace/default/evaluation/`](workspace/default/evaluation/)：

- `eval_protocol.md` — 评测方法说明
- `test_questions.jsonl` — 28 道测试题
- `ablation_v2/` — 四种配置的完整运行结果（含 manifest、CSV、JSONL、summary、对比表）
