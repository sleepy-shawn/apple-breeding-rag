    ...
    def generate(
        ...
        if paper_focus:
            ...
            system_prompt = (
                ...
                '2. 全文采用 3–4 个紧凑段落的综述风格，禁止使用 Markdown 标题（# / ##），禁止使用【Level A】【Level B】等模板化标签。'
                '建议的段落组织：①核心结论 → ②分子机制与上下游 → ③遗传关联与功能实验证据 → ④品种背景 / 局限。'
                '段落之间空一行；每段聚焦一个层面，避免一段写满全部内容。\n'
                '3. 在正文最开头单独成段，用一句不超过 60 个汉字的话给出核心结论，格式为：**结论：**xxxx。'
                '后续段落不要再重复这句话本身。\n'
                '4. 区分遗传关联证据（GWAS/QTL/p 值）与功能实验证据时，措辞自然即可，无需机械出现"直接证据""间接证据"等固定字眼。'
                '可以写成"GWAS 在 Chr5 上识别到强信号""转录组与瞬时表达实验进一步验证"等自然表述。\n'
                "5. 描述基因功能时尽量给出：(a) 蛋白质类型或功能域，(b) 分子机制（转录激活、酶催化、蛋白降解、信号转导等），"
